...
void scoreCalc( void )
{
    while (gameActive == 1)      //Signal recieved from button 
    {
        ...
        {
            lives--;
            SendMessage(addressLights, function, gameActive, lives, score); //Figure out score bytes
            SendMessage(addressMusic, function, gameActive, lives, score); //Figure out score bytes
        }
